...
#define MessageBoxF(...) \
	...
	MessageBox(NULL, g_msgBuffer, L"LichLoader", MB_OK | MB_ICONERROR);

#define AssertMessageBoxF(condition, ...) \
	if(!(condition)) { \
		swprintf(g_msgBuffer, MSGBUF_CHARS, __VA_ARGS__); \
		return MessageBox(NULL, g_msgBuffer, L"LichLoader", MB_OK | MB_ICONERROR); \
	}

#define DebugOutputF(...) \
	swprintf(g_msgBuffer, MSGBUF_CHARS, L"LichLoader: " __VA_ARGS__); \
